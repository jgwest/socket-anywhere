...
public class TestService implements Runnable {
	...
	/**
	 * Creates new TestService object, which will perform particular service on given socket.
	 * 
	 * @param s
	 ...
	 * @return Integer identifier for this service, or -1, if service can't be found.
	 */
	 ...
	 */
	public static boolean serve(Socket s, int service) throws IOException {
		switch (service) {
		case ECHO:
			echo(s);
			break;
	...
	/**
	 * Reads input from the socket, and does not write anything back. logs input in line by line fashion.
	 * 
	 * @param s
	 *            Socket on which to perform service.
	 */
	public static void discard(Socket s) throws IOException {
		BufferedReader in = new BufferedReader(new InputStreamReader(s.getInputStream()));
	 ...
	 * <p>
	 * Unlike usual chargen (port 19), each next line of the generated output is send after increasingly larger time
	 * intervals. It starts from wait_time (ms), and each next time wait time is doubled. Eg. 1 2 4 8 16 32 64 128 256
	 * 512 1024 2048 4096 8192 ... well you got the idea.
	 * <p>
	 * It starts if either connection is closed or the wait time grows bigger than max_wait.
	 * 
	 * @param s
	 *            Socket on which to perform service.
	 * @param wait_time
	 ...
	 * @param max_wait
	 *            Time in ms, after reaching timeout greater than this value, chargen will stop.
	 */
	 ...
	 * <p>
	 * Reads a line from the socket connection, line should be in the form port service_id. Connects back to the remote
	 * host to port specified in the line, if successful performs a service specified by id on that new connection. If
	 * accept was successful closes the control connection, else outputs error message, and then closes the connection.
	 * 
	 * @param s
	 *            Control connection.
	 */
	public static void connect(Socket s) throws IOException {
		String line = null;
		Socket sock;
		int port;
		int service_id;
