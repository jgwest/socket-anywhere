...
public class MNMultServerSocket implements IServerSocketTL {

	TLAddress _address;
	MNMultServerSocketListener _serverSocketListener;
	...
	protected MNMultServerSocket(MNConnectionBrain cb) {
		_cb = cb;
	}
	
	protected MNMultServerSocket(TLAddress addr, MNConnectionBrain cb) throws IOException {
		_cb = cb;
		bind(addr);
	}
	...
	public ISocketTL accept() throws IOException {
		if(!_isBound) {
			MnLog.error("Attempting to accept on a server socket that is not bound");
...
//		}
	...
	public void bind(TLAddress endpoint) throws IOException {
		...
		_serverSocketListener = new MNMultServerSocketListener(_address, _cb);
