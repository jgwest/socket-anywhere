...
public class LockManagerOld {
	...
	private static LockManagerOld _instance = new LockManagerOld();
...
//					System.out.println("adding lock" + lock.getName() + " ("+Thread.currentThread().getId()+")");
					...
					le._lock = lock;
					le._lockName = LockOld2.extractLockNameFromFilename(le._lock.getName());
					le._lockUUID = LockOld2.extractLockUUIDFromFilename(le._lock.getName());
	...
	public void removeLock(File lock) {
		synchronized(_activeLockFlush) { 
			synchronized (_waitingToRemoveLocks) { 
			...
			while(_lockManagerThreadRunning) {
				...
				synchronized (_activeLocks) {
					...
					for(LockEntry lock : _activeLocks) {
						File dir = lock._lock.getParentFile();
						...
						for(File f : fileList) {
							...
							if(f.getName().startsWith(lockTestFormat)) {
								String testUUID = LockOld2.extractTestUUIDFromFilename(f.getName());
